...
public class QueryChaincode {
	...
	public static void main(String args[]) {
		try {
			...
			byte[] channelConfig = channelClient.getChannel().getChannelConfigurationBytes();
			BlockchainInfo blockchainInfo = channel.queryBlockchainInfo();
			long blockHeight = blockchainInfo.getHeight();
			ByteString currentBlockHash = blockchainInfo.getBlockchainInfo().getPreviousBlockHash();
			BlockInfo blockInfo = channel.queryBlockByNumber(2);
			log.info(String.valueOf(Hex.toHexString(blockInfo.getDataHash())));
